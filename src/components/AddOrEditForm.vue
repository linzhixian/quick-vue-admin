        ...
        <!--新增和编辑界面-->        
            <el-form :model="addForm" label-width="100px" :rules="addFormRules" ref="addForm" :validate-on-rule-change="false" > 
                <el-form-item v-for="(oneItem,index) in editColumns" :prop="oneItem" :label="editColumnDefs[oneItem].label" :key="oneItem">
                    ...
                    <el-radio-group v-else-if="editColumnDefs[oneItem].input.type === 'radio'" v-model="addForm[oneItem]">
                    ...
                    <template v-else-if="editColumnDefs[oneItem].input.type==='twoText'">
                        <el-col :span="5">
                            <el-input v-model="addForm[editColumnDefs[oneItem].props[0]]"></el-input>
                        </el-col>
                        <el-col class="line" :span="1">-></el-col>
                        <el-col :span="5">
                            <el-input v-model="addForm[editColumnDefs[oneItem].props[1]]"></el-input>
                        </el-col>
                    </template>
                    <el-select v-else-if="editColumnDefs[oneItem].input.type==='selectMulti'" :filterable="editColumnDefs[oneItem].input.filterable" :allow-create="editColumnDefs[oneItem].input.allowCreate" :placeholder="editColumnDefs[oneItem].input.placeholder" v-model="addForm[oneItem]" clearable multiple>
                        <el-option v-for="one in editColumnDefs[oneItem].input.options" :label="one.label" :key="one.label" :value="one.value">
                        </el-option>
                    </el-select>
                    <el-select v-else-if="editColumnDefs[oneItem].input.type==='select'" v-model="addForm[oneItem]" clearable>
                        <el-option v-for="one in editColumnDefs[oneItem].input.options" :label="one.label" :key="one.label" :value="one.value">
                        </el-option>
                    </el-select>  
                  ...
                  <el-cascader v-if="editColumnDefs[oneItem].input.type==='cascader'" v-model="addForm[oneItem]"  :options="editColumnDefs[oneItem].input.options" :props="editColumnDefs[oneItem].input.props"  :placeholder="editColumnDefs[oneItem].input.placeholder" clearable filterable :change-on-select="editColumnDefs[oneItem].input.changeOnSelect"/> 
                    ...
                    <el-tree v-else-if="editColumnDefs[oneItem].input.type==='tree'" v-model="addForm[oneItem]" :data="editColumnDefs[oneItem].input.options.data" :props="editColumnDefs[oneItem].input.options.props" :ref="oneItem" node-key="label" show-checkbox>
                    </el-tree>

                    <el-date-picker v-if="editColumnDefs[oneItem].input.type==='dateRange'"
                 ...
                 <el-time-picker v-if="editColumnDefs[oneItem].input.type==='timeRange'"
                    is-range
                    v-model="addForm[oneItem]"
                    range-separator="至"
 ...
 export default {
    ...
    methods: {
      ...
      getFile(event,propName) { 
            ...
            if(colDef && colDef.input&& colDef.input.ext && colDef.input.ext!="*" && "."+colDef.input.ext!=extname) {
            ...
            for (let elem of columnDefs.values()) {        
                if (elem &&　elem.input) {
                  ...
                  this.editColumns.push(elem.prop);
                    ...
                    if (elem.input ) {
                        let defautValue=elem.input.default ? elem.input.default : "";
                ...
                if (valid) {
                        ...
                        if(!this.hasFileInput) {
                         ...
                         for(let k of Object.keys(this.addForm)) {
                            ...
                            if(typeof this.addForm[k]!='object' || this.addForm[k] instanceof File) {
